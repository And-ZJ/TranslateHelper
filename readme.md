...
本次更新包括
1. 更新版本号为0.8.7。
2. 新增对DeepL翻译网页的支持。不过还存在如下问题：
- 1） 点击格式化按钮，内容变了后，翻译结果不会自动更新。规避方法：点击多次格式化按钮，或者点击一下页面其他位置。
- 2） 隐藏了复制按钮，因为没有生效。规避方法：请点击DeepL页面自带的”复制“按钮图标。
3. 适配有道翻译新页面。
4. 修复在谷歌翻译页面的原文输入框被修改后，翻译结果不会自动更新的问题。
5. 升级到 jquery3.5.0。
...
1. 更新版本号0.8.6，因为上传的0.8.5版本在谷歌应用店解压失败。
